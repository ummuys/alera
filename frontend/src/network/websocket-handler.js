...
export class WebSocketHandler {
  constructor(ui, canvasHandler, options = {}) {
    ...
    this.requestedRoomName = options.requestedRoomName || options.requestedRoomId || '';
  ...
  handleSession(message) {
    ...
    this.ui.setRoomName(this.requestedRoomName || this.session.roomId);
    this.ui.addChatMessage(`Вы вошли как ${this.session.nickname}`, 'server');
  }
  ...
  handleUserJoined(message) {
    ...
    this.ui.addChatMessage(`${sender.nickname || 'Кто-то'} вошёл`, 'server');
  }

  handleUserLeft(message) {
    const sender = this.getSender(message);
    this.ui.addChatMessage(`${sender.nickname || 'Кто-то'} вышел`, 'server');
  }
